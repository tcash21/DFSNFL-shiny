...
if(length(which(is.na(preds$spread))) > 0){
        preds <- preds[-which(is.na(preds$spread)),]
}
...
shinyUI(pageWithSidebar(
  ...
  sidebarPanel(
     ...
     #sliderInput("value", "Value >= :", value=max(preds$value), min=min(preds$value), max=max(preds$value)),
     selectInput("type", "Objective", choices=c("NF"), selected="NF"),
  ...
  mainPanel(
	tabsetPanel(
		...
		tabPanel("All Players",dataTableOutput('allplayers')),
