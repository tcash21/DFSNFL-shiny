...
filterData <- reactive({
	...
	preds$df$name <- as.character(preds$df$player)
	...
	rhs <- c(1,1,1,2,1,3,rep(4,length(unique(preds2$team))),9,60000)
	result <- lp("max", obj, con, dir, rhs, all.bin = TRUE)	
	results <- preds2[which(result$solution == 1),]
	results$spread <- as.numeric(results$spread)
	#results$spread <- results$spread * -1
	results <- results[,c("name", "Position", "team","Opponent.x", "Salary", "nf_pred","line", "spread", "pa", "value")]
...
output$results <- renderDataTable({
	datatable(filterData()[[1]], options=list(autowidth=TRUE, selection="single", columnDefs = list(list(width='200px', targets="_all"))), rownames=TRUE, escape = FALSE)
})
